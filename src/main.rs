...
lazy_static! {
    static ref MESSAGE_QUEUE: Arc<Mutex<Vec<(String, String)>>> = {
        Arc::new(Mutex::new(vec!()))
    };
    static ref SEND_MESSAGE_QUEUE: Arc<Mutex<Vec<String>>> = {
        Arc::new(Mutex::new(vec!()))
    };
}
 ...
 fn chat(s: &mut Cursive, name: String, ip: String) {
     ...
     cursive.lock().unwrap().add_layer(Dialog::around(
         LinearLayout::vertical()
             ...
             .child(EditView::default().on_submit_mut(move |siv, _content| {
                 ...
                 compose.set_content("");
             }).with_name("compose")).scrollable()
     ).title("Chadder client").fixed_width(70).fixed_height(20));
