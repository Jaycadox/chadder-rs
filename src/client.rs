use crate::shared;
use crate::shared::{EncryptionInfo, Packet, COMPRESSION};
...
impl Client {
    ...
    fn handle_raw_packet(
        this: Arc<Mutex<Self>>,
        data: &[u8],
        encryption: Arc<Mutex<EncryptionInfo>>,
        ...
        if let Packet::Message(pack) = pack {
            this.lock()
                .unwrap()
                .message_received(pack.sender.clone(), pack.content.clone(), false);
            ...
            let key = match encryption
                ...
                .unwrap()
                .private
                .decrypt(PaddingScheme::new_pkcs1v15_encrypt(), &key)
            {
                Ok(key) => key,
            ...
            let nonce = match encryption
                ...
                .unwrap()
                .private
                .decrypt(PaddingScheme::new_pkcs1v15_encrypt(), &nonce)
            {
                Ok(nonce) => nonce,
            ...
            this.lock().unwrap().message_received(
                "Client".to_string(),
                "This connection is using end-to-end encryption.".to_string(),
                true,
            );
            let name = this.lock().unwrap().username.clone();
            this.lock()
                .unwrap()
                .send(Packet::Connection(shared::ConnectionPacket {
                    name,
                    secret: 46,
                }));
        }
    ...
    async fn handle_connection(
        ...
        if ENCRYPTION {
            this.lock()
                .unwrap()
                .send(Packet::RequestEncryption(shared::RequestEncryptionPacket {
        ...
        loop {
            ...
            match read.read(&mut buffer).await {
                ...
                Ok(n) => match Client::handle_raw_packet(Arc::clone(&this), &buffer[0..n], enc) {
                    Ok(_) => {}
                    Err(err) => {
    ...
    async fn handle_connection_writer(
        ...
        for message in enc.lock().unwrap().message_queue.clone() {
            ...
            if let Err(_) = write.write(&message).await {
                return false;
            };
        }
        true
    ...
    pub async fn start(this: Arc<Mutex<Self>>) -> Result<(), io::Error> {
        ...
        let addr: SocketAddr = format!("{}:{}", this.lock().unwrap().ip, PORT)
            .parse()
            .unwrap();
        ...
        tokio::spawn(async move {
            println!(
                ...
                Client::handle_connection(this_2, read, encryption_1).await
            );
        });
